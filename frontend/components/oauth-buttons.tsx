...
export default function OAuthButtons({ mode }: OAuthButtonsProps) {
  const { loginWithGoogle, loginWithMicrosoft, loginWithFacebook, loginWithApple } = useAuth();
  const { t } = useI18n();
  ...
  function handleFacebook() {
    if (!providers?.facebookAppId) return;
    setError("");
    const redirectUri = `${window.location.origin}/auth/facebook/callback`;
    const url =
      `https://www.facebook.com/v19.0/dialog/oauth?` +
      `client_id=${encodeURIComponent(providers.facebookAppId)}` +
      `&redirect_uri=${encodeURIComponent(redirectUri)}` +
      `&scope=email` +
      `&response_type=code`;
    window.location.href = url;
  ...
  const hasAnyProvider = providers.google || providers.microsoft || providers.apple || providers.facebook;
      ...
      <div className="mt-4 space-y-3">
        ...
        {providers.facebook && (
          <button
            ...
            className="flex w-full items-center justify-center gap-3 rounded-lg border border-zinc-300 bg-[#1877F2] px-4 py-2.5 text-sm font-medium text-white transition hover:bg-[#166FE5] disabled:opacity-50"
          >
            <svg className="h-5 w-5" viewBox="0 0 24 24" fill="currentColor">
