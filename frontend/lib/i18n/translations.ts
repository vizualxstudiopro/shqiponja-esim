const translations = {
  sq: {
    // Navbar
    "nav.packages": "Paketat",
    "nav.how": "Si Funksionon",
    "nav.operators": "Operatorët",
    "nav.login": "Kyçu",
    "nav.register": "Regjistrohu",
    "nav.logout": "Dil",
    "nav.profile": "Profili",
    "nav.install": "Instalimi",
    "nav.blog": "Blog",
    "nav.admin": "Admin",

    // Hero
    "hero.badge": "eSIM Ndërkombëtare — Pa kontratë",
    "hero.title1": "Lidhu me botën menjëherë me",
    "hero.title2": "Shqiponja eSIM",
    "hero.subtitle": "Merr internet të shpejtë kudo në botë — pa roaming, pa SIM fizike. Paketat tona ndërkombëtare ofrojnë mbulim në mbi 190 vende.",
    "hero.cta": "Bli Tani →",
    "hero.howLink": "Si Funksionon?",
    "hero.countries": "Vende",
    "hero.support": "Mbështetje",

    // Trust badges
    "trust.title": "Pse Shqiponja eSIM?",
    "trust.subtitle": "Klientë nga e gjithë bota na besojnë për internet të shpejtë kudo në botë.",
    "trust.instant": "Aktivizim i Menjëhershëm",
    "trust.instantDesc": "Merr QR kodin menjëherë pas pagesës — pa pritje, pa dërgesë.",
    "trust.secure": "Pagesë e Sigurt",
    "trust.secureDesc": "Pagesa procesohet në mënyrë të sigurt me enkriptim SSL.",
    "trust.global": "Mbulim Global",
    "trust.globalDesc": "Interneti funksionon në mbi 190 vende me 4G/5G.",
    "trust.noHidden": "Pa Tarifa të Fshehura",
    "trust.noHiddenDesc": "Çmimi që shikon është çmimi që paguan — zero surpriza.",
    "trust.airalo": "Partner Zyrtar i Airalo",
    "trust.airaloDesc": "Jemi partner i certifikuar i Airalo — platforma më e madhe e eSIM në botë.",
    "footer.airaloPartner": "Partner Zyrtar",

    // Testimonials
    "testimonials.title": "Çfarë thonë klientët tanë",
    "testimonials.subtitle": "Udhëtarë nga e gjithë bota na besojnë për internet pa ndërprerje kudo në botë.",
    "testimonials.review1": "E aktivizova eSIM-in para se të nisja fluturimin për në Greqi. Sapo mbërrita, interneti ishte gati. Fantastike!",
    "testimonials.review2": "Çmimet janë shumë të mira krahasuar me roaming-un. Tani e përdor çdo herë kur udhëtoj.",
    "testimonials.review3": "Si shqiptar në diasporë, kjo është zgjidhja perfekte kur kthehem në Shqipëri. Rekomandoj 100%.",
    "testimonials.review4": "Mbështetja në shqip bën diferencën e madhe. Kisha një problem dhe ma zgjidhën brenda minutave.",
    "testimonials.review5": "E përdora në Itali për 2 javë — shpejtësia ishte e shkëlqyer dhe kurrë nuk mbeta pa internet.",
    "testimonials.review6": "Procesi ishte i thjeshtë: bleva, skanova QR kodin, dhe u lidha. Nuk ka nevojë për SIM fizike!",

    // Operators
    "operators.title": "Mbështetur nga operatorët ndërkombëtarë",

    // Packages
    "packages.badge": "Paketat",
    "packages.title": "Zgjidh paketën e duhur për ty",
    "packages.subtitle": "Internet i shpejtë, çmime transparente, pa tarifa të fshehura.",
    "packages.popular": "Populare",
    "packages.data": "të dhëna",
    "packages.validity": "Vlefshmëria",
    "packages.buy": "Bli Tani",
    "packages.search": "Kërko paketa...",
    "packages.searchPlaceholder": "Kërko vendin ose rajonin...",
    "packages.emptyTitle": "Kërko destinacionin tënd",
    "packages.emptySubtitle": "Shkruaj emrin e vendit ose zgjedh rajonin për të parë paketat.",
    "packages.fetchError": "Nuk u arrit të ngarkoheshin paketat. Provo përsëri.",
    "packages.allRegions": "Të gjitha rajonet",
    "packages.sortDefault": "Renditje standarde",
    "packages.sortPriceAsc": "Çmimi: Ulët → Lartë",
    "packages.sortPriceDesc": "Çmimi: Lartë → Ulët",
    "packages.noResults": "Asnjë paketë nuk u gjet. Provo një kërkim tjetër.",
    "packages.local": "Lokale",
    "packages.regional": "Rajonale",
    "packages.global": "Globale",
    "packages.balkans": "Ballkani",
    "packages.europe": "Evropa",
    "packages.asia": "Azia",
    "packages.africa": "Afrika",
    "packages.americas": "Amerika",
    "packages.oceania": "Oqeania",
    "packages.middle_east": "Lindja e Mesme",
    "packages.viewAll": "Shiko të gjitha",
    "packages.popularDesc": "Destinacionet më të kërkuara — çmimet fillojnë nga çmimi i treguar.",
    "packages.localDesc": "Paketa eSIM për një vend të vetëm.",
    "packages.regionalDesc": "Paketa eSIM që mbulojnë një rajon të tërë.",
    "packages.globalDesc": "Paketa eSIM që mbulojnë botën.",
    "packages.balkansDesc": "Paketa eSIM për vendet e Ballkanit.",
    "packages.europeDesc": "Paketa eSIM për vendet Europiane.",
    "packages.asiaDesc": "Paketa eSIM për vendet Aziatike.",
    "packages.africaDesc": "Paketa eSIM për vendet Afrikane.",
    "packages.americasDesc": "Paketa eSIM për Amerikën Veriore dhe Jugore.",
    "packages.oceaniaDesc": "Paketa eSIM për Oqeani dhe Australi.",
    "packages.middle_eastDesc": "Paketa eSIM për vendet e Lindjes së Mesme.",
    "packages.searchAll": "Kërkon në të gjitha paketat e disponueshme",
    "packages.rateNote": "Kursi aktual:",

    // Package Finder
    "finder.badge": "Gjej Paketën",
    "finder.title": "Nuk e di cilën paketë të zgjedhësh?",
    "finder.subtitle": "Zgjedh kontinentin dhe vendin — ne të tregojmë paketat më të mira.",
    "finder.step1": "Zgjedh rajonin",
    "finder.step2": "Zgjedh vendin",
    "finder.step3": "Shiko paketat",
    "finder.global": "Globale",
    "finder.globalDesc": "Paketa që mbulojnë gjithë botën",
    "finder.balkans": "Ballkani",
    "finder.europe": "Evropa",
    "finder.asia": "Azia",
    "finder.middle_east": "Lindja e Mesme",
    "finder.africa": "Afrika",
    "finder.americas": "Amerika",
    "finder.oceania": "Oqeania",
    "finder.selectCountry": "Zgjedh vendin",
    "finder.packages": "paketa",
    "finder.from": "nga",
    "finder.loading": "Duke ngarkuar...",
    "finder.noPackages": "Asnjë paketë për këtë vend.",
    "finder.showAll": "Shiko të gjitha paketat",
    "finder.back": "Kthehu",
    "finder.resultsFor": "Paketat për",

    // World Cup 2026
    "worldcup.badge": "Botërori FIFA 2026",
    "worldcup.title": "Botërori i Futbollit 2026",
    "worldcup.subtitle": "SHBA, Kanada & Meksikë — siguro eSIM-in tënd para se të nisesh për ndeshjet!",
    "worldcup.buy": "Bli eSIM",
    "worldcup.from": "Nga",
    "worldcup.allPackages": "Shiko të gjitha paketat",
    "worldcup.days": "Ditë",
    "worldcup.hours": "Orë",
    "worldcup.minutes": "Minuta",
    "worldcup.seconds": "Sekonda",

    // Tourist destinations
    "tourist.badge": "Destinacione Turistike",
    "tourist.title": "Destinacionet më të preferuara",
    "tourist.subtitle": "Paketa eSIM me çmimet më të mira për destinacionet turistike më të njohura.",
    "tourist.buy": "Bli",
    "tourist.tagBeach": "Plazh",
    "tourist.tagCulture": "Kulturë",
    "tourist.tagHistory": "Histori",
    "tourist.tagExotic": "Ekzotik",
    "tourist.tagAdventure": "Aventurë",
    "tourist.share": "Shpërnda",

    // How it works
    "how.badge": "3 Hapa",
    "how.title": "Si funksionon?",
    "how.subtitle": "Nga blerja deri në aktivizim — vetëm tre hapa të thjeshtë.",
    "how.step": "Hapi",
    "how.step1.title": "Zgjidh Planin",
    "how.step1.desc": "Shfleto paketat tona dhe zgjidh atë që të përshtatet.",
    "how.step2.title": "Skano QR Code",
    "how.step2.desc": "Merre QR kodin përmes email-it dhe skanoje me telefonin.",
    "how.step3.title": "Aktivizo Internetin",
    "how.step3.desc": "Aktivizo eSIM-in direkt nga cilësimet e telefonit dhe shijo internetin.",

    // Device Compatibility
    "compat.badge": "Kompatibilitëti",
    "compat.title": "A është pajisja jote e gatshme?",
    "compat.subtitle": "Shkruaj modelin e telefonit tënd për të verifikuar nëse mbështet eSIM.",
    "compat.placeholder": "p.sh. iPhone 13, Samsung S24, Pixel 8...",
    "compat.yes": "Po, pajisja juaj është e gatshme për eSIM!",
    "compat.unknown": "Nuk e gjetëm këtë model. Kontrolloni listen zyrtare të prodhuesit.",
    "compat.hint": "Lista përfshijn shumicën e pajisjeve të njohura. Nëse nuk e gjeni tuajën, kontaktoni prodhuesin.",
    "compat.showAll": "Shiko të gjitha pajisjet",
    "compat.modalTitle": "Pajisjet e mbështetura me eSIM",

    // CTA
    "cta.title": "Gati për të udhëtuar pa kufij?",
    "cta.subtitle": "Bli paketën tënde eSIM sot dhe shijo internet kudo pa roaming.",
    "cta.btn": "Shiko Paketat →",

    // Footer
    "footer.rights": "© 2026 Shqiponja eSIM. Të gjitha të drejtat e rezervuara.",
    "footer.operatedBy": "operohet nga",
    "footer.terms": "Kushtet e Përdorimit",
    "footer.privacy": "Privatësia",
    "footer.contact": "Kontakti",
    "footer.refund": "Rimbursimet",
    "footer.cookies": "Politika e Cookies",
    "footer.companyDetails": "Të Dhënat e Kompanisë",
    "footer.addressCountry": "United States",
    "footer.legalInfo": "Informacioni Ligjor",
    "footer.legalNameLabel": "Emri Ligjor",
    "footer.einLabel": "EIN",
    "footer.payWith": "Paguaj me siguri",
    "footer.paymentsComingSoon": "Integrimi i pagesave vjen së shpejti.",
    "footer.appsComingSoon": "Aplikacionet iOS dhe Android vijnë së shpejti.",
    "footer.navigation": "Navigimi",
    "footer.legal": "Ligjore",

    // Refund Policy
    "refund.title": "Politika e Rimbursimit dhe Kthimit",
    "refund.lastUpdated": "Përditësuar për herë të fundit: 1 Maj 2026",
    "refund.intro": "Në Shqiponja eSIM (operuar nga VALA TECH 2026 LLC), përpiqemi të ofrojmë shërbime të shkëlqyera dixhitale të telekomunikacionit. Meqenëse produktet tona janë dixhitale (profile eSIM), politika jonë e rimbursimit ndjek rreptësisht udhëzimet për mallrat dixhitale.",
    "refund.s1.title": "1. Kualifikimi për Rimbursim",
    "refund.s1.text": "Ju kualifikoheni për rimbursim të plotë VETËM në kushtet e mëposhtme:",
    "refund.s1.item1": "QR kodi i eSIM-it NUK është skanuar, instaluar, ose aktivizuar në pajisjen tuaj.",
    "refund.s1.item2": "Kërkesa për rimbursim bëhet brenda 14 ditëve nga blerja origjinale.",
    "refund.s1.item3": "Ka një dështim teknik të verifikuar nga ana e ofruesit tonë të rrjetit (Airalo) që parandalon lidhjen e eSIM-it, dhe ekipi ynë i mbështetjes nuk është në gjendje ta zgjidhë çështjen.",
    "refund.s2.title": "2. Situata të Parimbursushme",
    "refund.s2.text": "NUK mund të lëshojmë rimbursim në rastet e mëposhtme:",
    "refund.s2.item1": "eSIM-i është instaluar dhe aktivizuar në pajisjen tuaj (edhe nëse të dhënat nuk janë përdorur).",
    "refund.s2.item2": "Pajisja juaj është e bllokuar nga një operator specifik ose nuk është e përputhshme me eSIM. (Është përgjegjësia e klientit të kontrollojë përputhshmërinë e pajisjes përpara blerjes).",
    "refund.s2.item3": "Keni ndërruar mendje pasi eSIM-i është instaluar.",
    "refund.s2.item4": "Keni fshirë ose hequr profilin eSIM nga pajisja juaj pas instalimit.",
    "refund.s3.title": "3. Si të Kërkoni Rimbursim",
    "refund.s3.text": "Për të kërkuar rimbursim, ju lutem kontaktoni ekipin tonë të mbështetjes në info@shqiponjaesim.com me ID-në e Porosisë tuaj dhe një përshkrim të detajuar të problemit. Ekipi ynë do të verifikojë statusin e aktivizimit të eSIM-it me ofruesin tonë të rrjetit përpara miratimit të rimbursimit.",
    "refund.s3.processing": "Rimbursimet e miratuara do të procesohen përmes metodës origjinale të pagesës brenda 5-10 ditëve pune.",

    // Auth
    "auth.loginTitle": "Kyçu në llogari",
    "auth.loginSubtitle": "Fut email-in dhe fjalëkalimin tënd",
    "auth.email": "Email",
    "auth.password": "Fjalëkalimi",
    "auth.loginBtn": "Kyçu",
    "auth.loggingIn": "Duke u kyçur...",
    "auth.noAccount": "Nuk ke llogari?",
    "auth.registerTitle": "Krijo llogari të re",
    "auth.registerSubtitle": "Regjistrohu për të menaxhuar porositë e tua",
    "auth.name": "Emri i plotë",
    "auth.confirmPassword": "Konfirmo fjalëkalimin",
    "auth.registerBtn": "Regjistrohu",
    "auth.registering": "Duke u regjistruar...",
    "auth.hasAccount": "Ke llogari tashmë?",
    "auth.passwordMismatch": "Fjalëkalimet nuk përputhen",

    // Forgot/Reset Password
    "auth.forgotLink": "Ke harruar fjalëkalimin?",
    "auth.forgotTitle": "Rivendos fjalëkalimin",
    "auth.forgotSubtitle": "Fut email-in tënd dhe do të marrësh një link",
    "auth.forgotBtn": "Dërgo linkun",
    "auth.forgotSent": "Nëse ky email ekziston në sistemin tonë, do të marrësh një link për rivendosjen e fjalëkalimit.",
    "auth.resetTitle": "Vendos fjalëkalimin e ri",
    "auth.newPassword": "Fjalëkalimi i ri",
    "auth.resetBtn": "Rivendos",
    "auth.resetSuccess": "Fjalëkalimi u ndryshua me sukses!",
    "auth.resetNoToken": "Link i pavlefshëm. Kërko një link të ri.",

    // OAuth
    "auth.orContinueWith": "ose vazhdo me",
    "auth.continueWith": "Vazhdo me",
    "auth.signUpWith": "Regjistrohu me",

    // 2FA
    "auth.totpCode": "Kodi 2FA",
    "auth.totpHint": "Fut kodin 6-shifror nga aplikacioni juaj autentifikues",

    // About & FAQ
    "nav.about": "Rreth Nesh",
    "nav.faq": "FAQ",
    "about.title": "Rreth Shqiponja eSIM",
    "about.mission": "Misioni ynë është të lidhim shqiptarët me botën — pa kufij, pa roaming, pa SIM fizike.",
    "about.story": "Shqiponja eSIM u themelua me vizionin për të ofruar internet të shpejtë dhe të përballueshëm për udhëtarët shqiptarë kudo në botë.",
    "about.values.title": "Vlerat Tona",
    "about.values.1": "Transparencë — çmime të qarta, pa tarifa të fshehura",
    "about.values.2": "Cilësi — mbulim në mbi 190 vende me 4G/5G",
    "about.values.3": "Mbështetje — ekip i dedikuar për klientët",
    "faq.title": "Pyetje të Shpeshta",
    "faq.q1": "Çfarë është eSIM?",
    "faq.a1": "eSIM është një SIM dixhitale që të lejon të aktivizosh një plan celular pa pasur nevojë për SIM fizike.",
    "faq.q2": "Si e instaloj eSIM-in?",
    "faq.a2": "Pas blerjes, do të marrësh një QR kod. Skanoje nga Cilësimet > Celular > Shto Plan eSIM në telefonin tënd.",
    "faq.q3": "A funksionon me telefonin tim?",
    "faq.a3": "eSIM funksionon me shumicën e telefonave të rinj: iPhone XS+, Samsung Galaxy S20+, Google Pixel 3+ etj.",
    "faq.q4": "A mund ta përdor njëkohësisht me SIM-in fizike?",
    "faq.a4": "Po! Shumica e telefonave mbështesin Dual SIM — mund ta përdorësh eSIM-in për internet dhe SIM fizike për thirrje.",
    "faq.q5": "Çfarë ndodh kur perfundon plani?",
    "faq.a5": "Kur përfundon plani, interneti ndalet automatikisht. Mund të blesh një plan të ri në çdo kohë.",

    // Dark mode
    "theme.light": "Dritë",
    "theme.dark": "Errët",

    // Profile
    "profile.client": "Klient",
    "profile.memberSince": "Anëtar që nga",
    "profile.myOrders": "Porositë e Mia",
    "profile.noOrders": "Nuk ke asnjë porosi ende",
    "profile.viewPackages": "Shiko Paketat",
    "profile.order": "Porosi",
    "profile.paid": "E paguar",
    "profile.pending": "Në pritje",
    "profile.completed": "Kompletuar",
    "profile.editName": "Ndrysho Emrin",
    "profile.changePassword": "Ndrysho Fjalëkalimin",
    "profile.newName": "Emri i ri",
    "profile.save": "Ruaj",
    "profile.currentPassword": "Fjalëkalimi aktual",
    "profile.newPassword": "Fjalëkalimi i ri",
    "profile.nameUpdated": "Emri u përditësua me sukses",
    "profile.passwordChanged": "Fjalëkalimi u ndryshua me sukses",

    // SMS 2FA
    "profile.sms2fa.title": "Siguria me SMS (2FA)",
    "profile.sms2fa.subtitle": "Aktivizo verifikimin me dy hapa. Çdo herë që hyni, do ju dërgohet nje kod në telefonin tuaj.",
    "profile.sms2fa.enabled": "2FA me SMS është aktive",
    "profile.sms2fa.disabled": "2FA me SMS nuk është aktive",
    "profile.sms2fa.phone": "Numri i telefonit",
    "profile.sms2fa.phonePlaceholder": "+355 69 XXX XXXX",
    "profile.sms2fa.sendCode": "Dërgo Kod",
    "profile.sms2fa.sending": "Duke dërguar...",
    "profile.sms2fa.codeSent": "Kodi u dërgua!",
    "profile.sms2fa.enterCode": "Fut kodin 6-shifror",
    "profile.sms2fa.activate": "Aktivizo 2FA",
    "profile.sms2fa.activating": "Duke aktivizuar...",
    "profile.sms2fa.activated": "2FA me SMS u aktivizua!",
    "profile.sms2fa.disable": "Deaktivizo 2FA",
    "profile.sms2fa.disabled_ok": "2FA me SMS u deaktivizua",
    "profile.sms2fa.maskedPhone": "Tel:",
    "auth.smsCode": "Kodi SMS",
    "auth.smsCodeHint": "Kontrolloni telefonin tuaj, ju dërguam nje kod 6-shifror.",

    // Referral
    "referral.title": "Fto Miqtë",
    "referral.subtitle": "Ndajeni kodin tuaj me miqtë dhe fitoni zbritje për çdo blerje të tyre.",
    "referral.copyLink": "Kopjo Linkun",
    "referral.copied": "U kopjua!",
    "referral.invited": "Të ftuar",
    "referral.completed": "Të kompletuar",
    "referral.earned": "Fituar",

    // 404
    "notFound.title": "Faqja nuk u gjet",
    "notFound.subtitle": "Faqja që po kërkon nuk ekziston ose është zhvendosur.",
    "notFound.home": "Kthehu në Faqen Kryesore",

    // Terms
    "terms.title": "Kushtet e Përdorimit",
    "terms.lastUpdated": "Përditësuar për herë të fundit: 1 Maj 2026",
    "terms.operator": "Operatori: Ky shërbim ofrohet nga VALA TECH 2026 LLC, Wyoming, USA.",
    "terms.s1.title": "1. Pranimi i kushteve",
    "terms.s1.text": "Duke përdorur Shqiponja eSIM, ju pranoni këto kushte përdorimi.",
    "terms.s2.title": "2. Shërbimi",
    "terms.s2.text": "Shqiponja eSIM ofron paketa dixhitale eSIM për internet mobil ndërkombëtar.",
    "terms.s3.title": "3. Pagesat dhe rimbursimet",
    "terms.s3.text": "Pagesat përpunohen nga partnerët tanë të pagesave. Rimbursimet zbatohen sipas politikës së kthimeve.",
    "terms.s4.title": "4. Përgjegjësia",
    "terms.s4.text": "Nuk mbajmë përgjegjësi për ndërprerje të rrjeteve të palëve të treta jashtë kontrollit tonë.",
    "terms.s5.title": "5. Ndryshimet e kushteve",
    "terms.s5.text": "Rezervojmë të drejtën të përditësojmë këto kushte në çdo kohë.",
    "terms.s6.title": "6. Age Restriction & Applicable Law",
    "terms.s6.text": "You must be at least 18 years old to purchase. These Terms are governed by the laws of the State of Wyoming, USA. Any dispute will be resolved exclusively in the state or federal courts located in Cheyenne, Wyoming.",
    "terms.disclaimer.title": "Disclaimer për eSIM",
    "terms.disclaimer.item1": "eSIM është produkt dixhital, jo kartë fizike.",
    "terms.disclaimer.item2": "Shërbimi është vetëm për të dhëna interneti, jo për thirrje/SMS tradicionale.",
    "terms.disclaimer.item3pre": "Klienti duhet të kontrollojë kompatibilitetin e pajisjes",
    "terms.disclaimer.item3link": "këtu",
    "terms.disclaimer.item3post": "përpara blerjes.",
    "terms.disclaimer.item4": "Nevojitet lidhje interneti për aktivizimin fillestar të eSIM-it.",

    // Privacy
    "privacy.title": "Politika e Privatësisë",
    "privacy.lastUpdated": "Përditësuar për herë të fundit: 1 Maj 2026",
    "privacy.s1.title": "1. Të dhënat që mbledhim",
    "privacy.s1.text": "Mbledhim të dhëna bazë si emri, email-i, informacioni i porosisë dhe të dhëna teknike të nevojshme për funksionimin e shërbimit Shqiponja eSIM.",
    "privacy.s2.title": "2. Si i përdorim të dhënat",
    "privacy.s2.text": "Të dhënat përdoren për procesimin e porosive, dërgimin e eSIM, mbështetje ndaj klientit, parandalim abuzimi dhe përmirësim të platformës.",
    "privacy.s2.legal": "Baza ligjore për përpunimin: Përpunojmë të dhënat tuaja mbi bazën e ekzekutimit të kontratës kur blini një eSIM, interesit legjitim për parandalimin e mashtrimit dhe sigurinë e platformës, dhe pëlqimit tuaj për komunikime marketingu dhe cookies jo-esenciale.",
    "privacy.s3.title": "3. Ruajtja dhe siguria",
    "privacy.s3.text": "Aplikojmë masa teknike dhe organizative të arsyeshme për të mbrojtur të dhënat personale dhe për të kufizuar qasjen e paautorizuar.",
    "privacy.s4.title": "4. Cookies",
    "privacy.s4.text": "Përdorim cookies esenciale, analitike dhe funksionale sipas zgjedhjes suaj të konsentit. Për detaje, shihni Cookie Policy.",
    "privacy.s5.title": "5. Kontakt",
    "privacy.controller.title": "Kontrolluesi i të Dhënave",
    "privacy.controller.text": "VALA TECH 2026 LLC, 2232 Dell Range Blvd, Suite 303 1440, Cheyenne, WY 82009, USA.",
    "privacy.transfer.title": "Transferimi i të Dhënave",
    "privacy.transfer.text": "Të dhënat personale mund të përpunohen dhe ruhen në Shtetet e Bashkuara të Amerikës (USA).",
    "privacy.gdpr.title": "Të Drejtat Tuaja GDPR",
    "privacy.gdpr.text": "Ju keni të drejtë për akses, korrigjim dhe fshirje të të dhënave tuaja personale. Për ushtrimin e këtyre të drejtave, na kontaktoni në",
    "privacy.retention.title": "Data Retention",
    "privacy.retention.text": "We retain order and billing data for 7 years to comply with IRS tax law. Customer support messages are deleted after 90 days. Analytics cookies expire after 26 months.",
    "privacy.children.title": "Children’s Privacy",
    "privacy.children.text": "Our service is not directed to users under 18. We do not knowingly collect personal data from minors. If you believe a child provided data, contact us at",
    "privacy.children.suffix": "for deletion.",
    "privacy.supervisory.title": "Supervisory Authority",
    "privacy.supervisory.text": "EU users have the right to lodge a complaint with your local data protection authority if you believe we violated GDPR.",
    "privacy.payments.title": "Pagesat",
    "privacy.payments.text": "Pagesat procesohen nga ofrues të certifikuar të pagesave. Ne nuk ruajmë të dhëna të kartave bankare në sistemet tona.",
    "privacy.payments.nosell": "Nuk i shesim, japim me qira, ose ndajmë të dhënat tuaja personale me palë të treta për qëllime marketingu.",

    // Cookies
    "cookies.title": "Cookie Policy",
    "cookies.lastUpdated": "Përditësuar: 1 Maj 2026",
    "cookies.s1.title": "Përdorimi i Cookies",
    "cookies.s1.intro": "VALA TECH 2026 LLC, operator i markës Shqiponja eSIM, përdor cookies për funksionimin e faqes, matjen e performancës dhe përmirësimin e eksperiencës së përdoruesit.",
    "cookies.s1.item1": "Cookies esenciale për funksionimin bazë të platformës.",
    "cookies.s1.item2": "Cookies analitike nga Google Analytics.",
    "cookies.s1.item3": "Cookies marketingu nga Meta Pixel.",
    "cookies.s1.item4": "Cookies funksionale për chat dhe mbështetje klienti.",
    "cookies.s2.title": "Menaxhimi i Cookies",
    "cookies.s2.text": "Ju mund të pranoni ose refuzoni cookies jo-esenciale përmes banner-it të cookies. Zgjedhja ruhet në pajisjen tuaj dhe mund të ndryshohet duke fshirë të dhënat lokale të shfletuesit.",
    "cookies.s3.title": "Palët e Treta",
    "cookies.s3.text": "Shërbimet e mëposhtme mund të vendosin ose lexojnë cookies sipas funksionalitetit të tyre:",
    "cookies.s4.title": "Kontakt",
    "cookies.contact.address": "Adresa: VALA TECH 2026 LLC, 2232 Dell Range Blvd, Suite 303 1440, Cheyenne, WY 82009, USA",

    // Imprint
    "imprint.title": "Të Dhënat e Kompanisë (Imprint)",
    "imprint.legal.title": "Informacioni Ligjor",
    "imprint.legal.item1": "Emri Ligjor: VALA TECH 2026 LLC",
    "imprint.legal.item2": "Employer Identification Number: 37-2233850",
    "imprint.legal.item3": "Brand: Shqiponja eSIM",
    "imprint.legal.item4": "Përgjegjës: VALA TECH 2026 LLC",
    "imprint.legal.item5": "Shteti i Regjistrimit: Wyoming, USA",
    "imprint.contact.title": "Adresa dhe Kontaktet",
    "imprint.contact.address": "Adresa: 2232 Dell Range Blvd, Suite 303 1440, Cheyenne, WY 82009, United States",
    "imprint.dispute.title": "Zgjidhja e Mosmarrëveshjeve",
    "imprint.dispute.text": "Për çdo ankesë ose mosmarrëveshje, ju lutem na kontaktoni në",
    "imprint.dispute.suffix": "Ne përgjigjemi brenda 48 orësh.",

    // Contact
    "contact.title": "Na Kontaktoni",
    "contact.subtitle": "Keni pyetje? Plotësoni formën dhe do t'ju përgjigjemi sa më shpejt.",
    "contact.name": "Emri juaj",
    "contact.email": "Email-i juaj",
    "contact.message": "Mesazhi",
    "contact.send": "Dërgo Mesazhin",
    "contact.sending": "Po dërgohet...",
    "contact.required": "Të gjitha fushat janë të detyrueshme",
    "contact.success": "Mesazhi u dërgua me sukses!",
    "contact.error": "Dërgimi dështoi. Provoni përsëri.",
    "contact.sent": "Faleminderit! Mesazhi juaj u dërgua me sukses.",
    "contact.confirmEmail": "Do të merrni një email konfirmimi së shpejti.",
    "contact.sendAnother": "Dërgo mesazh tjetër",
    "contact.namePlaceholder": "Emri dhe mbiemri",
    "contact.emailPlaceholder": "email@shembull.com",
    "contact.messagePlaceholder": "Shkruani mesazhin tuaj këtu...",

    // Admin
    "admin.dashboard": "Dashboard",
    "admin.packages": "Paketat",
    "admin.orders": "Porositë",
    "admin.users": "Përdoruesit",
    "admin.backToSite": "Kthehu në faqe",
    "admin.refresh": "Rifresko",
    "admin.summary": "Përmbledhje e platformës",
    "admin.totalOrders": "Porosi Totale",
    "admin.paidOrders": "Porosi të Paguara",
    "admin.revenue": "Të Ardhura",
    "admin.usersCount": "Përdorues",
    "admin.packagesCount": "Paketa",
    "admin.monthlyRevenue": "Të Ardhura Mujore",
    "admin.monthlyOrders": "Porosi Mujore",
    "admin.monthlyUsers": "Përdorues të Rinj Mujorë",
    "admin.noData": "Nuk ka të dhëna ende",
    "admin.totalSuffix": "gjithsej",
    "admin.package": "Paketa",
    "admin.email": "Email",
    "admin.payment": "Pagesa",
    "admin.status": "Statusi",
    "admin.date": "Data",
    "admin.action": "Veprim",
    "admin.actions": "Veprime",
    "admin.paid": "E paguar",
    "admin.unpaid": "Në pritje",
    "admin.completed": "Kompletuar",
    "admin.pendingStatus": "Në pritje",
    "admin.markUnpaid": "Shëno si papaguar",
    "admin.markPaid": "Shëno si paguar",
    "admin.noOrders": "Asnjë porosi ende",
    "admin.page": "Faqja",
    "admin.name": "Emri",
    "admin.role": "Roli",
    "admin.registered": "Regjistruar",
    "admin.admin": "Admin",
    "admin.customer": "Klient",
    "admin.makeCustomer": "Bëje klient",
    "admin.makeAdmin": "Bëje admin",
    "admin.delete": "Fshi",
    "admin.confirmDelete": "Je i sigurt që dëshiron ta fshish",
    "admin.addPackage": "+ Shto Paketë",
    "admin.phone": "Nr. Tel",
    "admin.marketing": "Marketing",
    "admin.marketingEmail": "Dërgo Email",
    "admin.marketingSms": "Dërgo SMS",
    "admin.marketingType": "Lloji",
    "admin.marketingSubject": "Subjekti",
    "admin.marketingMessage": "Mesazhi",
    "admin.marketingRecipients": "Destinatarët",
    "admin.marketingAll": "Të gjithë",
    "admin.marketingSelected": "Të zgjedhur",
    "admin.marketingSend": "Dërgo",
    "admin.marketingSending": "Duke dërguar...",
    "admin.marketingResult": "Dërguar: {sent}, Dështuan: {failed}",
    "admin.marketingNoPhone": "Asnjë përdorues me numër telefoni",
    "admin.twilioBalance": "Bilanci Twilio",
    "admin.twilioBalanceLoading": "Duke ngarkuar...",
    "admin.twilioBalanceError": "Nuk u ngarkua",
    "admin.region": "Rajoni",
    "admin.data": "Të dhëna",
    "admin.duration": "Kohëzgjatja",
    "admin.price": "Çmimi",
    "admin.edit": "Ndrysho",
    "admin.editPackage": "Ndrysho Paketën",
    "admin.addPackageTitle": "Shto Paketë",
    "admin.description": "Përshkrimi",
    "admin.popular": "Populare (highlight)",
    "admin.cancel": "Anulo",
    "admin.save": "Ruaj",
    "admin.confirmDeletePkg": "Je i sigurt?",
    "admin.search": "Kërko...",
    "admin.exportCSV": "Eksporto CSV",
    "admin.allStatuses": "Të gjitha statuset",
    "admin.filterByStatus": "Filtro sipas statusit",
    "admin.showOnWeb": "Shto në Web",
    "admin.hideFromWeb": "Hiq nga Web",
    "admin.visibleOnWeb": "Aktiv në Web",
    "admin.hiddenFromWeb": "I fshehur",
    "admin.cancelled": "Anuluar",
    "admin.refunded": "Rimbursuar",
    "admin.customers": "Klientët",
    "admin.settings": "Konfigurimet",

    // Admin Promo Codes
    "admin.promo.title": "Promo Kodet",
    "admin.promo.subtitle": "Menaxho kodet e zbritjes dhe kuponat",
    "admin.promo.add": "Shto Kupon",
    "admin.promo.code": "Kodi",
    "admin.promo.type": "Lloji",
    "admin.promo.percent": "Përqindje",
    "admin.promo.fixed": "Fikse",
    "admin.promo.value": "Vlera",
    "admin.promo.discount": "Zbritja",
    "admin.promo.usage": "Përdorimi",
    "admin.promo.maxUses": "Max përdorime",
    "admin.promo.minOrder": "Min porosi (€)",
    "admin.promo.expires": "Skadon",
    "admin.promo.status": "Statusi",
    "admin.promo.create": "Krijo Kuponin",
    "admin.promo.created": "Kuponi u krijua me sukses!",
    "admin.promo.deleted": "Kuponi u fshi!",
    "admin.promo.confirmDelete": "Je i sigurt që do ta fshish këtë kupon?",
    "admin.promo.empty": "Nuk ka kupona ende",

    // Admin Referrals
    "admin.ref.title": "Referimet",
    "admin.ref.subtitle": "Shiko dhe menaxho programin e referimeve",
    "admin.ref.referrer": "Referi",
    "admin.ref.referred": "I referuari",
    "admin.ref.reward": "Shpërblimi",
    "admin.ref.status": "Statusi",
    "admin.ref.date": "Data",
    "admin.ref.allStatuses": "Të gjitha statuset",
    "admin.ref.total": "gjithsej",
    "admin.ref.totalReferrals": "Referime totale",
    "admin.ref.completedReferrals": "Të kompletuar",
    "admin.ref.totalRewards": "Shpërblime totale",
    "admin.ref.updated": "Statusi u përditësua!",
    "admin.ref.empty": "Nuk ka referime ende",

    // Admin Security / 2FA
    "admin.security": "Siguria",
    "admin.securityDesc": "Menaxho sigurinë e llogarisë suaj admin",
    "admin.twoFactor": "Autentifikimi me dy faktorë (2FA)",
    "admin.twoFactorDesc": "Shto një shtresë shtesë sigurie me Google Authenticator ose ndonjë aplikacion tjetër TOTP",
    "admin.twoFactorOn": "Aktive",
    "admin.twoFactorOff": "Joaktive",
    "admin.enable2FA": "Aktivizo 2FA",
    "admin.scanQR": "Skanoni këtë QR kod me aplikacionin tuaj autentifikues:",
    "admin.manualKey": "Ose futni këtë çelës manualisht:",
    "admin.enterCode": "Futni kodin 6-shifror nga aplikacioni:",
    "admin.verify2FA": "Verifiko dhe Aktivizo",
    "admin.disable2FATitle": "Çaktivizo 2FA",
    "admin.disable2FA": "Çaktivizo 2FA",

    // Buy page
    "buy.data": "Të dhëna",
    "buy.duration": "Kohëzgjatja",
    "buy.price": "Çmimi",
    "buy.fillOrder": "Plotëso Porosinë",
    "buy.fillOrderSub": "Fut të dhënat e tua dhe do të marrësh QR kodin për eSIM-in.",
    "buy.nameLabel": "Emri dhe Mbiemri",
    "buy.namePlaceholder": "p.sh. Filan Fisteku",
    "buy.emailLabel": "Adresa Email",
    "buy.emailPlaceholder": "emri@shembull.com",
    "buy.phoneLabel": "Numri i Telefonit (opsional)",
    "buy.phonePlaceholder": "+355 69 123 4567",
    "buy.processing": "Duke procesuar...",
    "buy.pay": "Paguaj",
    "buy.termsAgree": "Pranoj",
    "buy.termsLink": "kushtet e përdorimit",
    "buy.andText": "dhe",
    "buy.privacyLink": "politikën e privatësisë",
    "buy.termsRequired": "Duhet të pranoni kushtet për të vazhduar.",
    "buy.emailRequired": "Email-i është i detyrueshëm.",
    "buy.nameRequired": "Emri është i detyrueshëm.",
    "buy.error": "Diçka shkoi keq.",
    "buy.orderSummary": "Përmbledhja e Porosisë",
    "buy.securePayment": "Pagesë e sigurt",
    "buy.backToPackages": "← Kthehu te paketat",
    "promo.label": "Kod Zbritjeje",
    "promo.placeholder": "p.sh. SHQIPONJA20",
    "promo.apply": "Apliko",
    "promo.applied": "Kodi u aplikua!",
    "promo.saved": "Kurseve",
    "promo.invalid": "Ky kod nuk është i vlefshëm",
    "blog.title": "Blog",
    "blog.subtitle": "Artikuj dhe udhëzues rreth eSIM, udhëtimit, dhe teknologjisë.",
    "blog.notFound": "Artikulli nuk u gjet",
    "blog.backToBlog": "Kthehu te Blog",
    "blog.cta": "Gati për të provën eSIM? Shikoni paketat tona.",
    "blog.viewPackages": "Shiko Paketat",
    "blog.related": "Artikuj të ngjashëm",

    // Order confirmation page
    "order.title": "Porosia u Regjistrua!",
    "order.subtitle": "Faleminderit për porosinë tënde. QR kodi i eSIM-it do të dërgohet në email-in tënd.",
    "order.orderId": "Porosi",
    "order.email": "Email",
    "order.payment": "Pagesa",
    "order.status": "Statusi",
    "order.date": "Data",
    "order.paid": "E paguar",
    "order.pending": "Në pritje",
    "order.completed": "Kompletuar",
    "order.scanQr": "Skanoje këtë QR kod me cilësimet e telefonit tënd për ta aktivizuar eSIM-in.",
    "order.qrPending": "QR Kodi do të shfaqet këtu pasi pagesa të konfirmohet",
    "order.backHome": "← Kthehu në Faqen Kryesore",
    "order.downloadInvoice": "Shkarko Faturën",
    "order.dataUsage": "Përdorimi i të Dhënave",
    "order.used": "Përdorur",
    "order.loading": "Duke ngarkuar porosinë...",
    "order.notFound": "Porosia nuk u gjet",

    // Profile extras
    "profile.emailVerified": "Email i verifikuar",
    "profile.emailNotVerified": "Email i pa-verifikuar",
    "profile.resendVerification": "Ridërgo verifikimin",
    "profile.verificationSent": "Email-i i verifikimit u dërgua!",

    // Cookie consent
    "cookie.title": "Përdorimi i cookie-ve",
    "cookie.description": "Përdorim cookie për autentifikim, preferencat e gjuhës dhe analitikë. Nuk përdorim cookie për reklama.",
    "cookie.accept": "Pranoj",
    "cookie.decline": "Refuzoj",
    "cookie.learnMore": "Mëso më shumë",

    // eSIM Installation Guide
    "install.badge": "Guida e Instalimit",
    "install.title": "Si të instaloni eSIM-in",
    "install.subtitle": "Ndiqni hapat e thjeshtë më poshtë për të aktivizuar eSIM-in tuaj në iPhone ose Android.",
    "install.iphone.step1": "Hapni email-in tuaj dhe gjeni porosinë me QR kodin e eSIM-it.",
    "install.iphone.step2": "Shkoni te Cilësimet → Celulare (ose Të Dhënat Mobile) → Shto Plan Celulare.",
    "install.iphone.step3": "Skanoni QR kodin me kamerën e telefonit. Sigurohuni që jeni të lidhur me Wi-Fi.",
    "install.iphone.step4": "Klikoni \"Vazhdo\" kur t'ju pyesë — zgjidhni \"Sekondare\" ose \"Travel\" si etiketë.",
    "install.iphone.step5": "Aktivizoni \"Roaming i të Dhënave\" → Cilësimet → Celulare → eSIM-i juaj → Roaming.",
    "install.iphone.step6": "Zgjidhni eSIM-in si linjë për Të Dhënat Celulare. Gati! Keni internet.",
    "install.android.step1": "Hapni email-in tuaj dhe gjeni porosinë me QR kodin e eSIM-it.",
    "install.android.step2": "Shkoni te Cilësimet → Rrjeti & Interneti → SIM-et → Shto eSIM (ose + Shto).",
    "install.android.step3": "Zgjidhni \"Skano QR kodin\" dhe skanoni kodin nga email-i. Duhet Wi-Fi.",
    "install.android.step4": "Konfirmoni shkarkim-in e profilit të eSIM-it duke klikuar \"Vazhdo\" ose \"Shkarko\".",
    "install.android.step5": "Aktivizoni eSIM-in e re → Cilësimet → Rrjeti & Interneti → SIM-et → Aktivizo Roaming.",
    "install.android.step6": "Zgjidhni eSIM-in si linjë për të dhënat mobile. Gati! Interneti është aktiv.",
    "install.tipsTitle": "Këshilla të Rëndësishme",
    "install.tip1": "Sigurohuni që telefoni është i lidhur me Wi-Fi gjatë instalimit.",
    "install.tip2": "Mos e fshini eSIM-in pasi ta keni instaluar — nuk mund ta ri-përdorni QR kodin.",
    "install.tip3": "Aktivizoni gjithmonë \"Roaming i të Dhënave\" për eSIM-in.",
    "install.tip4": "eSIM-i mund të instalohet përpara udhëtimit — aktivizoni kur të mbërrini.",
    "install.troubleTitle": "Probleme me Instalimin?",
    "install.trouble1": "Sigurohuni që pajisja juaj mbështet eSIM (iPhone XS+, Samsung S20+, Pixel 3+).",
    "install.trouble2": "Kontrolloni që telefoni nuk është i bllokuar nga operatori (carrier-locked).",
    "install.trouble3": "QR kodi duhet të skanohet vetëm 1 herë — nuk mund të ri-përdoret.",
    "install.trouble4": "Nëse nuk funksionon, kontaktoni mbështetjen tonë përmes faqes së kontaktit.",
  },
  en: {
    // Navbar
    "nav.packages": "Packages",
    "nav.how": "How It Works",
    "nav.operators": "Operators",
    "nav.login": "Log In",
    "nav.register": "Sign Up",
    "nav.logout": "Log Out",
    "nav.profile": "Profile",
    "nav.install": "Install Guide",
    "nav.blog": "Blog",
    "nav.admin": "Admin",

    // Hero
    "hero.badge": "International eSIM — No contract",
    "hero.title1": "Connect to the world instantly with",
    "hero.title2": "Shqiponja eSIM",
    "hero.subtitle": "Get fast internet anywhere in the world — no roaming, no physical SIM. Our international packages cover 190+ countries.",
    "hero.cta": "Buy Now →",
    "hero.howLink": "How It Works?",
    "hero.countries": "Countries",
    "hero.clients": "Clients",
    "hero.support": "Support",

    // Trust badges
    "trust.title": "Why Shqiponja eSIM?",
    "trust.subtitle": "Customers from around the world trust us for fast internet wherever they travel.",
    "trust.instant": "Instant Activation",
    "trust.instantDesc": "Get your QR code immediately after payment — no waiting, no shipping.",
    "trust.secure": "Secure Payment",
    "trust.secureDesc": "Payments are processed securely with SSL encryption.",
    "trust.global": "Global Coverage",
    "trust.globalDesc": "Internet works in 190+ countries with 4G/5G.",
    "trust.noHidden": "No Hidden Fees",
    "trust.noHiddenDesc": "The price you see is the price you pay — zero surprises.",
    "trust.airalo": "Official Airalo Partner",
    "trust.airaloDesc": "We are a certified partner of Airalo — the world's largest eSIM platform.",
    "footer.airaloPartner": "Official Partner",

    // Testimonials
    "testimonials.title": "What our customers say",
    "testimonials.subtitle": "Travelers from around the world trust us for seamless internet wherever they go.",
    "testimonials.review1": "I activated the eSIM before my flight to Greece. As soon as I landed, the internet was ready. Fantastic!",
    "testimonials.review2": "The prices are much better compared to roaming. Now I use it every time I travel.",
    "testimonials.review3": "As an Albanian in the diaspora, this is the perfect solution when I visit home. 100% recommend.",
    "testimonials.review4": "Support in Albanian makes a huge difference. I had an issue and they solved it within minutes.",
    "testimonials.review5": "I used it in Italy for 2 weeks — the speed was excellent and I never ran out of internet.",
    "testimonials.review6": "The process was simple: bought it, scanned the QR code, and connected. No need for a physical SIM!",

    // Operators
    "operators.title": "Supported by international operators",

    // Packages
    "packages.badge": "Packages",
    "packages.title": "Choose the right package for you",
    "packages.subtitle": "Fast internet, transparent prices, no hidden fees.",
    "packages.popular": "Popular",
    "packages.data": "data",
    "packages.validity": "Validity",
    "packages.buy": "Buy Now",
    "packages.search": "Search packages...",
    "packages.searchPlaceholder": "Search country or region...",
    "packages.emptyTitle": "Search your destination",
    "packages.emptySubtitle": "Type a country name or select a region to see packages.",
    "packages.fetchError": "Could not load packages. Please try again.",
    "packages.allRegions": "All regions",
    "packages.sortDefault": "Default order",
    "packages.sortPriceAsc": "Price: Low → High",
    "packages.sortPriceDesc": "Price: High → Low",
    "packages.noResults": "No packages found. Try a different search.",
    "packages.local": "Local",
    "packages.regional": "Regional",
    "packages.global": "Global",
    "packages.balkans": "Balkans",
    "packages.europe": "Europe",
    "packages.asia": "Asia",
    "packages.africa": "Africa",
    "packages.americas": "Americas",
    "packages.oceania": "Oceania",
    "packages.middle_east": "Middle East",
    "packages.viewAll": "View all locations",
    "packages.popularDesc": "Most popular destinations — prices start from the shown price.",
    "packages.localDesc": "eSIM packages for a single country.",
    "packages.regionalDesc": "eSIM packages covering an entire region.",
    "packages.globalDesc": "eSIM packages with worldwide coverage.",
    "packages.balkansDesc": "eSIM packages for Balkan countries.",
    "packages.europeDesc": "eSIM packages for European countries.",
    "packages.asiaDesc": "eSIM packages for Asian countries.",
    "packages.africaDesc": "eSIM packages for African countries.",
    "packages.americasDesc": "eSIM packages for North and South America.",
    "packages.oceaniaDesc": "eSIM packages for Oceania and Australia.",
    "packages.middle_eastDesc": "eSIM packages for Middle Eastern countries.",
    "packages.searchAll": "Searching across all available packages",
    "packages.rateNote": "Current rate:",

    // Package Finder
    "finder.badge": "Find Package",
    "finder.title": "Not sure which package to choose?",
    "finder.subtitle": "Select a continent and country — we'll show you the best packages.",
    "finder.step1": "Select region",
    "finder.step2": "Select country",
    "finder.step3": "View packages",
    "finder.global": "Global",
    "finder.globalDesc": "Packages that cover the whole world",
    "finder.balkans": "Balkans",
    "finder.europe": "Europe",
    "finder.asia": "Asia",
    "finder.middle_east": "Middle East",
    "finder.africa": "Africa",
    "finder.americas": "Americas",
    "finder.oceania": "Oceania",
    "finder.selectCountry": "Select country",
    "finder.packages": "packages",
    "finder.from": "from",
    "finder.loading": "Loading...",
    "finder.noPackages": "No packages available for this country.",
    "finder.showAll": "View all packages",
    "finder.back": "Go back",
    "finder.resultsFor": "Packages for",

    // World Cup 2026
    "worldcup.badge": "FIFA World Cup 2026",
    "worldcup.title": "FIFA World Cup 2026",
    "worldcup.subtitle": "USA, Canada & Mexico — get your eSIM before heading to the matches!",
    "worldcup.buy": "Buy eSIM",
    "worldcup.from": "From",
    "worldcup.allPackages": "View all packages",
    "worldcup.days": "Days",
    "worldcup.hours": "Hours",
    "worldcup.minutes": "Minutes",
    "worldcup.seconds": "Seconds",

    // Tourist destinations
    "tourist.badge": "Tourist Destinations",
    "tourist.title": "Most popular destinations",
    "tourist.subtitle": "eSIM packages with the best prices for the most popular tourist destinations.",
    "tourist.buy": "Buy",
    "tourist.tagBeach": "Beach",
    "tourist.tagCulture": "Culture",
    "tourist.tagHistory": "History",
    "tourist.tagExotic": "Exotic",
    "tourist.tagAdventure": "Adventure",
    "tourist.share": "Share",

    // How it works
    "how.badge": "3 Steps",
    "how.title": "How it works?",
    "how.subtitle": "From purchase to activation — just three simple steps.",
    "how.step": "Step",
    "how.step1.title": "Choose a Plan",
    "how.step1.desc": "Browse our packages and choose the one that fits you.",
    "how.step2.title": "Scan QR Code",
    "how.step2.desc": "Receive the QR code via email and scan it with your phone.",
    "how.step3.title": "Activate Internet",
    "how.step3.desc": "Activate the eSIM from your phone settings and enjoy the internet.",

    // Device Compatibility
    "compat.badge": "Compatibility",
    "compat.title": "Is your device ready?",
    "compat.subtitle": "Type your phone model to check if it supports eSIM.",
    "compat.placeholder": "e.g. iPhone 13, Samsung S24, Pixel 8...",
    "compat.yes": "Yes, your device is eSIM ready!",
    "compat.unknown": "We couldn\u2019t find this model. Check your manufacturer\u2019s official list.",
    "compat.hint": "This list covers most popular devices. If yours isn\u2019t listed, contact your manufacturer.",
    "compat.showAll": "Show all devices",
    "compat.modalTitle": "eSIM supported devices",

    // CTA
    "cta.title": "Ready to travel without limits?",
    "cta.subtitle": "Buy your eSIM package today and enjoy internet everywhere without roaming.",
    "cta.btn": "View Packages →",

    // Footer
    "footer.rights": "© 2026 Shqiponja eSIM. All rights reserved.",
    "footer.operatedBy": "Operated by",
    "footer.terms": "Terms of Use",
    "footer.privacy": "Privacy",
    "footer.contact": "Contact",
    "footer.refund": "Refund Policy",
    "footer.cookies": "Cookie Policy",
    "footer.companyDetails": "Company Details",
    "footer.addressCountry": "United States",
    "footer.legalInfo": "Legal Information",
    "footer.legalNameLabel": "Legal Name",
    "footer.einLabel": "EIN",
    "footer.payWith": "Pay securely with",
    "footer.paymentsComingSoon": "Payment integration is coming soon.",
    "footer.appsComingSoon": "iOS and Android apps are coming soon.",
    "footer.navigation": "Navigation",
    "footer.legal": "Legal",

    // Refund Policy
    "refund.title": "Refund and Returns Policy",
    "refund.lastUpdated": "Last updated: May 1, 2026",
    "refund.intro": "At Shqiponja eSIM (operated by VALA TECH 2026 LLC), we strive to provide excellent digital telecommunication services. Because our products are digital (eSIM profiles), our refund policy strictly follows the guidelines for digital goods.",
    "refund.s1.title": "1. Eligibility for Refund",
    "refund.s1.text": "You are eligible for a full refund ONLY under the following conditions:",
    "refund.s1.item1": "The eSIM QR code has NOT been scanned, installed, or activated on your device.",
    "refund.s1.item2": "The refund request is made within 14 days of the original purchase.",
    "refund.s1.item3": "There is a verified technical failure on the part of our network provider (Airalo) that prevents the eSIM from connecting, and our support team is unable to resolve the issue.",
    "refund.s2.title": "2. Non-Refundable Situations",
    "refund.s2.text": "We CANNOT issue a refund in the following cases:",
    "refund.s2.item1": "The eSIM has been installed and activated on your device (even if data was not used).",
    "refund.s2.item2": "Your device is locked to a specific carrier or is not eSIM-compatible. (It is the customer's responsibility to check device compatibility before purchase).",
    "refund.s2.item3": "You have changed your mind after the eSIM has been installed.",
    "refund.s2.item4": "You have deleted or removed the eSIM profile from your device after installation.",
    "refund.s3.title": "3. How to Request a Refund",
    "refund.s3.text": "To request a refund, please contact our support team at info@shqiponjaesim.com with your Order ID and a detailed description of the issue. Our team will verify the activation status of the eSIM with our network provider before approving the refund.",
    "refund.s3.processing": "Approved refunds will be processed back to the original method of payment within 5-10 business days.",

    // Auth
    "auth.loginTitle": "Log in to your account",
    "auth.loginSubtitle": "Enter your email and password",
    "auth.email": "Email",
    "auth.password": "Password",
    "auth.loginBtn": "Log In",
    "auth.loggingIn": "Logging in...",
    "auth.noAccount": "Don't have an account?",
    "auth.registerTitle": "Create a new account",
    "auth.registerSubtitle": "Sign up to manage your orders",
    "auth.name": "Full name",
    "auth.confirmPassword": "Confirm password",
    "auth.registerBtn": "Sign Up",
    "auth.registering": "Signing up...",
    "auth.hasAccount": "Already have an account?",
    "auth.passwordMismatch": "Passwords don't match",

    // Forgot/Reset Password
    "auth.forgotLink": "Forgot your password?",
    "auth.forgotTitle": "Reset your password",
    "auth.forgotSubtitle": "Enter your email and we'll send you a link",
    "auth.forgotBtn": "Send link",
    "auth.forgotSent": "If this email exists in our system, you will receive a password reset link.",
    "auth.resetTitle": "Set a new password",
    "auth.newPassword": "New password",
    "auth.resetBtn": "Reset",
    "auth.resetSuccess": "Password changed successfully!",
    "auth.resetNoToken": "Invalid link. Request a new one.",

    // OAuth
    "auth.orContinueWith": "or continue with",
    "auth.continueWith": "Continue with",
    "auth.signUpWith": "Sign up with",

    // 2FA
    "auth.totpCode": "2FA Code",
    "auth.totpHint": "Enter the 6-digit code from your authenticator app",

    // About & FAQ
    "nav.about": "About Us",
    "nav.faq": "FAQ",
    "about.title": "About Shqiponja eSIM",
    "about.mission": "Our mission is to connect Albanians with the world — no borders, no roaming, no physical SIM.",
    "about.story": "Shqiponja eSIM was founded with the vision to provide fast and affordable internet for Albanian travelers worldwide.",
    "about.values.title": "Our Values",
    "about.values.1": "Transparency — clear prices, no hidden fees",
    "about.values.2": "Quality — coverage in 190+ countries with 4G/5G",
    "about.values.3": "Support — dedicated customer support team",
    "faq.title": "Frequently Asked Questions",
    "faq.q1": "What is an eSIM?",
    "faq.a1": "An eSIM is a digital SIM that lets you activate a cellular plan without needing a physical SIM card.",
    "faq.q2": "How do I install the eSIM?",
    "faq.a2": "After purchase, you'll receive a QR code. Scan it from Settings > Cellular > Add eSIM Plan on your phone.",
    "faq.q3": "Does it work with my phone?",
    "faq.a3": "eSIM works with most newer phones: iPhone XS+, Samsung Galaxy S20+, Google Pixel 3+, etc.",
    "faq.q4": "Can I use it alongside my physical SIM?",
    "faq.a4": "Yes! Most phones support Dual SIM — you can use the eSIM for data and keep your physical SIM for calls.",
    "faq.q5": "What happens when the plan expires?",
    "faq.a5": "When the plan expires, data stops automatically. You can buy a new plan at any time.",

    // Dark mode
    "theme.light": "Light",
    "theme.dark": "Dark",

    // Profile
    "profile.client": "Client",
    "profile.memberSince": "Member since",
    "profile.myOrders": "My Orders",
    "profile.noOrders": "You don't have any orders yet",
    "profile.viewPackages": "View Packages",
    "profile.order": "Order",
    "profile.paid": "Paid",
    "profile.pending": "Pending",
    "profile.completed": "Completed",
    "profile.editName": "Edit Name",
    "profile.changePassword": "Change Password",
    "profile.newName": "New name",
    "profile.save": "Save",
    "profile.currentPassword": "Current password",
    "profile.newPassword": "New password",
    "profile.nameUpdated": "Name updated successfully",
    "profile.passwordChanged": "Password changed successfully",

    // SMS 2FA
    "profile.sms2fa.title": "SMS Security (2FA)",
    "profile.sms2fa.subtitle": "Enable two-step verification. Every time you log in, a code will be sent to your phone.",
    "profile.sms2fa.enabled": "SMS 2FA is active",
    "profile.sms2fa.disabled": "SMS 2FA is not active",
    "profile.sms2fa.phone": "Phone number",
    "profile.sms2fa.phonePlaceholder": "+355 69 XXX XXXX",
    "profile.sms2fa.sendCode": "Send Code",
    "profile.sms2fa.sending": "Sending...",
    "profile.sms2fa.codeSent": "Code sent!",
    "profile.sms2fa.enterCode": "Enter 6-digit code",
    "profile.sms2fa.activate": "Activate 2FA",
    "profile.sms2fa.activating": "Activating...",
    "profile.sms2fa.activated": "SMS 2FA activated!",
    "profile.sms2fa.disable": "Disable 2FA",
    "profile.sms2fa.disabled_ok": "SMS 2FA disabled",
    "profile.sms2fa.maskedPhone": "Phone:",
    "auth.smsCode": "SMS Code",
    "auth.smsCodeHint": "Check your phone, we sent you a 6-digit code.",

    // Referral
    "referral.title": "Invite Friends",
    "referral.subtitle": "Share your code with friends and earn discounts for every purchase they make.",
    "referral.copyLink": "Copy Link",
    "referral.copied": "Copied!",
    "referral.invited": "Invited",
    "referral.completed": "Completed",
    "referral.earned": "Earned",

    // 404
    "notFound.title": "Page not found",
    "notFound.subtitle": "The page you're looking for doesn't exist or has been moved.",
    "notFound.home": "Back to Home",

    // Terms
    "terms.title": "Terms of Use",
    "terms.lastUpdated": "Last updated: May 1, 2026",
    "terms.operator": "Operator: This service is provided by VALA TECH 2026 LLC, Wyoming, USA.",
    "terms.s1.title": "1. Acceptance of Terms",
    "terms.s1.text": "By using Shqiponja eSIM, you accept these terms of use.",
    "terms.s2.title": "2. Service",
    "terms.s2.text": "Shqiponja eSIM offers digital eSIM packages for international mobile internet.",
    "terms.s3.title": "3. Payments and Refunds",
    "terms.s3.text": "Payments are processed by our payment partners. Refunds are applied according to the returns policy.",
    "terms.s4.title": "4. Liability",
    "terms.s4.text": "We are not liable for interruptions from third-party networks outside our control.",
    "terms.s5.title": "5. Changes to Terms",
    "terms.s5.text": "We reserve the right to update these terms at any time.",
    "terms.s6.title": "6. Age Restriction & Applicable Law",
    "terms.s6.text": "You must be at least 18 years old to purchase. These Terms are governed by the laws of the State of Wyoming, USA. Any dispute will be resolved exclusively in the state or federal courts located in Cheyenne, Wyoming.",
    "terms.disclaimer.title": "eSIM Disclaimer",
    "terms.disclaimer.item1": "eSIM is a digital product, not a physical card.",
    "terms.disclaimer.item2": "The service is for internet data only, not traditional calls/SMS.",
    "terms.disclaimer.item3pre": "The customer must check device compatibility",
    "terms.disclaimer.item3link": "here",
    "terms.disclaimer.item3post": "before purchasing.",
    "terms.disclaimer.item4": "An internet connection is required for the initial eSIM activation.",

    // Privacy
    "privacy.title": "Privacy Policy",
    "privacy.lastUpdated": "Last updated: May 1, 2026",
    "privacy.s1.title": "1. Data We Collect",
    "privacy.s1.text": "We collect basic data such as name, email, order information and technical data required for the Shqiponja eSIM service to function.",
    "privacy.s2.title": "2. How We Use Data",
    "privacy.s2.text": "Data is used to process orders, deliver the eSIM, provide customer support, prevent abuse and improve the platform.",
    "privacy.s2.legal": "Legal basis for processing: We process your data based on contract execution when you purchase an eSIM, legitimate interest in fraud prevention and platform security, and your consent for marketing communications and non-essential cookies.",
    "privacy.s3.title": "3. Storage and Security",
    "privacy.s3.text": "We apply reasonable technical and organizational measures to protect personal data and limit unauthorized access.",
    "privacy.s4.title": "4. Cookies",
    "privacy.s4.text": "We use essential, analytical and functional cookies based on your consent choices. For details, see the Cookie Policy.",
    "privacy.s5.title": "5. Contact",
    "privacy.controller.title": "Data Controller",
    "privacy.controller.text": "VALA TECH 2026 LLC, 2232 Dell Range Blvd, Suite 303 1440, Cheyenne, WY 82009, USA.",
    "privacy.transfer.title": "Data Transfer",
    "privacy.transfer.text": "Personal data may be processed and stored in the United States of America (USA).",
    "privacy.gdpr.title": "Your GDPR Rights",
    "privacy.gdpr.text": "You have the right to access, correct and delete your personal data. To exercise these rights, contact us at",
    "privacy.retention.title": "Data Retention",
    "privacy.retention.text": "We retain order and billing data for 7 years to comply with IRS tax law. Customer support messages are deleted after 90 days. Analytics cookies expire after 26 months.",
    "privacy.children.title": "Children’s Privacy",
    "privacy.children.text": "Our service is not directed to users under 18. We do not knowingly collect personal data from minors. If you believe a child provided data, contact us at",
    "privacy.children.suffix": "for deletion.",
    "privacy.supervisory.title": "Supervisory Authority",
    "privacy.supervisory.text": "EU users have the right to lodge a complaint with your local data protection authority if you believe we violated GDPR.",
    "privacy.payments.title": "Payments",
    "privacy.payments.text": "Payments are processed by certified payment providers. We do not store card data in our systems.",
    "privacy.payments.nosell": "We do not sell, rent or share your personal data with third parties for marketing purposes.",

    // Cookies
    "cookies.title": "Cookie Policy",
    "cookies.lastUpdated": "Updated: May 1, 2026",
    "cookies.s1.title": "Use of Cookies",
    "cookies.s1.intro": "VALA TECH 2026 LLC, operator of the Shqiponja eSIM brand, uses cookies for site functionality, performance measurement and user experience improvement.",
    "cookies.s1.item1": "Essential cookies for basic platform functionality.",
    "cookies.s1.item2": "Analytical cookies from Google Analytics.",
    "cookies.s1.item3": "Marketing cookies from Meta Pixel.",
    "cookies.s1.item4": "Functional cookies for chat and customer support.",
    "cookies.s2.title": "Cookie Management",
    "cookies.s2.text": "You can accept or reject non-essential cookies through the cookie banner. Your choice is saved on your device and can be changed by clearing your browser's local data.",
    "cookies.s3.title": "Third Parties",
    "cookies.s3.text": "The following services may set or read cookies according to their functionality:",
    "cookies.s4.title": "Contact",
    "cookies.contact.address": "Address: VALA TECH 2026 LLC, 2232 Dell Range Blvd, Suite 303 1440, Cheyenne, WY 82009, USA",

    // Imprint
    "imprint.title": "Company Information (Imprint)",
    "imprint.legal.title": "Legal Information",
    "imprint.legal.item1": "Legal Name: VALA TECH 2026 LLC",
    "imprint.legal.item2": "Employer Identification Number: 37-2233850",
    "imprint.legal.item3": "Brand: Shqiponja eSIM",
    "imprint.legal.item4": "Responsible: VALA TECH 2026 LLC",
    "imprint.legal.item5": "State of Registration: Wyoming, USA",
    "imprint.contact.title": "Address and Contact",
    "imprint.contact.address": "Address: 2232 Dell Range Blvd, Suite 303 1440, Cheyenne, WY 82009, United States",
    "imprint.dispute.title": "Dispute Resolution",
    "imprint.dispute.text": "For any complaint or dispute, please contact us at",
    "imprint.dispute.suffix": "We respond within 48 hours.",

    // Contact
    "contact.title": "Contact Us",
    "contact.subtitle": "Have questions? Fill out the form and we'll get back to you as soon as possible.",
    "contact.name": "Your name",
    "contact.email": "Your email",
    "contact.message": "Message",
    "contact.send": "Send Message",
    "contact.sending": "Sending...",
    "contact.required": "All fields are required",
    "contact.success": "Message sent successfully!",
    "contact.error": "Sending failed. Please try again.",
    "contact.sent": "Thank you! Your message has been sent successfully.",
    "contact.confirmEmail": "You will receive a confirmation email shortly.",
    "contact.sendAnother": "Send another message",
    "contact.namePlaceholder": "First and last name",
    "contact.emailPlaceholder": "email@example.com",
    "contact.messagePlaceholder": "Write your message here...",

    // Admin
    "admin.dashboard": "Dashboard",
    "admin.packages": "Packages",
    "admin.orders": "Orders",
    "admin.users": "Users",
    "admin.backToSite": "Back to site",
    "admin.refresh": "Refresh",
    "admin.summary": "Platform overview",
    "admin.totalOrders": "Total Orders",
    "admin.paidOrders": "Paid Orders",
    "admin.revenue": "Revenue",
    "admin.usersCount": "Users",
    "admin.packagesCount": "Packages",
    "admin.monthlyRevenue": "Monthly Revenue",
    "admin.monthlyOrders": "Monthly Orders",
    "admin.monthlyUsers": "New Users Monthly",
    "admin.noData": "No data yet",
    "admin.totalSuffix": "total",
    "admin.package": "Package",
    "admin.email": "Email",
    "admin.payment": "Payment",
    "admin.status": "Status",
    "admin.date": "Date",
    "admin.action": "Action",
    "admin.actions": "Actions",
    "admin.paid": "Paid",
    "admin.unpaid": "Pending",
    "admin.completed": "Completed",
    "admin.pendingStatus": "Pending",
    "admin.markUnpaid": "Mark as unpaid",
    "admin.markPaid": "Mark as paid",
    "admin.noOrders": "No orders yet",
    "admin.page": "Page",
    "admin.name": "Name",
    "admin.role": "Role",
    "admin.registered": "Registered",
    "admin.admin": "Admin",
    "admin.customer": "Customer",
    "admin.makeCustomer": "Make customer",
    "admin.makeAdmin": "Make admin",
    "admin.delete": "Delete",
    "admin.confirmDelete": "Are you sure you want to delete",
    "admin.addPackage": "+ Add Package",
    "admin.phone": "Phone",
    "admin.marketing": "Marketing",
    "admin.marketingEmail": "Send Email",
    "admin.marketingSms": "Send SMS",
    "admin.marketingType": "Type",
    "admin.marketingSubject": "Subject",
    "admin.marketingMessage": "Message",
    "admin.marketingRecipients": "Recipients",
    "admin.marketingAll": "All",
    "admin.marketingSelected": "Selected",
    "admin.marketingSend": "Send",
    "admin.marketingSending": "Sending...",
    "admin.marketingResult": "Sent: {sent}, Failed: {failed}",
    "admin.marketingNoPhone": "No users with phone number",
    "admin.twilioBalance": "Twilio Balance",
    "admin.twilioBalanceLoading": "Loading...",
    "admin.twilioBalanceError": "Failed to load",
    "admin.region": "Region",
    "admin.data": "Data",
    "admin.duration": "Duration",
    "admin.price": "Price",
    "admin.edit": "Edit",
    "admin.editPackage": "Edit Package",
    "admin.addPackageTitle": "Add Package",
    "admin.description": "Description",
    "admin.popular": "Popular (highlight)",
    "admin.cancel": "Cancel",
    "admin.save": "Save",
    "admin.confirmDeletePkg": "Are you sure?",
    "admin.search": "Search...",
    "admin.exportCSV": "Export CSV",
    "admin.allStatuses": "All statuses",
    "admin.filterByStatus": "Filter by status",
    "admin.showOnWeb": "Add to Web",
    "admin.hideFromWeb": "Remove from Web",
    "admin.visibleOnWeb": "Live on Web",
    "admin.hiddenFromWeb": "Hidden",
    "admin.cancelled": "Cancelled",
    "admin.refunded": "Refunded",
    "admin.customers": "Customers",
    "admin.settings": "Settings",

    // Admin Promo Codes
    "admin.promo.title": "Promo Codes",
    "admin.promo.subtitle": "Manage discount codes and coupons",
    "admin.promo.add": "Add Coupon",
    "admin.promo.code": "Code",
    "admin.promo.type": "Type",
    "admin.promo.percent": "Percentage",
    "admin.promo.fixed": "Fixed",
    "admin.promo.value": "Value",
    "admin.promo.discount": "Discount",
    "admin.promo.usage": "Usage",
    "admin.promo.maxUses": "Max uses",
    "admin.promo.minOrder": "Min order (€)",
    "admin.promo.expires": "Expires",
    "admin.promo.status": "Status",
    "admin.promo.create": "Create Coupon",
    "admin.promo.created": "Coupon created successfully!",
    "admin.promo.deleted": "Coupon deleted!",
    "admin.promo.confirmDelete": "Are you sure you want to delete this coupon?",
    "admin.promo.empty": "No coupons yet",

    // Admin Referrals
    "admin.ref.title": "Referrals",
    "admin.ref.subtitle": "View and manage the referral program",
    "admin.ref.referrer": "Referrer",
    "admin.ref.referred": "Referred",
    "admin.ref.reward": "Reward",
    "admin.ref.status": "Status",
    "admin.ref.date": "Date",
    "admin.ref.allStatuses": "All statuses",
    "admin.ref.total": "total",
    "admin.ref.totalReferrals": "Total referrals",
    "admin.ref.completedReferrals": "Completed",
    "admin.ref.totalRewards": "Total rewards",
    "admin.ref.updated": "Status updated!",
    "admin.ref.empty": "No referrals yet",

    // Admin Security / 2FA
    "admin.security": "Security",
    "admin.securityDesc": "Manage your admin account security",
    "admin.twoFactor": "Two-Factor Authentication (2FA)",
    "admin.twoFactorDesc": "Add an extra layer of security with Google Authenticator or any TOTP app",
    "admin.twoFactorOn": "Enabled",
    "admin.twoFactorOff": "Disabled",
    "admin.enable2FA": "Enable 2FA",
    "admin.scanQR": "Scan this QR code with your authenticator app:",
    "admin.manualKey": "Or enter this key manually:",
    "admin.enterCode": "Enter the 6-digit code from your app:",
    "admin.verify2FA": "Verify and Enable",
    "admin.disable2FATitle": "Disable 2FA",
    "admin.disable2FA": "Disable 2FA",

    // Buy page
    "buy.data": "Data",
    "buy.duration": "Duration",
    "buy.price": "Price",
    "buy.fillOrder": "Complete Your Order",
    "buy.fillOrderSub": "Enter your details and you'll receive the QR code for the eSIM.",
    "buy.nameLabel": "Full Name",
    "buy.namePlaceholder": "e.g. John Doe",
    "buy.emailLabel": "Email Address",
    "buy.emailPlaceholder": "name@example.com",
    "buy.phoneLabel": "Phone Number (optional)",
    "buy.phonePlaceholder": "+1 234 567 8900",
    "buy.processing": "Processing...",
    "buy.pay": "Pay",
    "buy.termsAgree": "I agree to the",
    "buy.termsLink": "terms of use",
    "buy.andText": "and",
    "buy.privacyLink": "privacy policy",
    "buy.termsRequired": "You must agree to the terms to continue.",
    "buy.emailRequired": "Email is required.",
    "buy.nameRequired": "Name is required.",
    "buy.error": "Something went wrong.",
    "buy.orderSummary": "Order Summary",
    "buy.securePayment": "Secure payment",
    "buy.backToPackages": "← Back to packages",
    "promo.label": "Promo Code",
    "promo.placeholder": "e.g. SHQIPONJA20",
    "promo.apply": "Apply",
    "promo.applied": "Code applied!",
    "promo.saved": "You saved",
    "promo.invalid": "This code is not valid",
    "blog.title": "Blog",
    "blog.subtitle": "Articles and guides about eSIM, travel, and technology.",
    "blog.notFound": "Article not found",
    "blog.backToBlog": "Back to Blog",
    "blog.cta": "Ready to try eSIM? Check out our packages.",
    "blog.viewPackages": "View Packages",
    "blog.related": "Related articles",

    // Order confirmation page
    "order.title": "Order Confirmed!",
    "order.subtitle": "Thank you for your order. The eSIM QR code will be sent to your email.",
    "order.orderId": "Order",
    "order.email": "Email",
    "order.payment": "Payment",
    "order.status": "Status",
    "order.date": "Date",
    "order.paid": "Paid",
    "order.pending": "Pending",
    "order.completed": "Completed",
    "order.scanQr": "Scan this QR code with your phone settings to activate the eSIM.",
    "order.qrPending": "QR Code will appear here once payment is confirmed",
    "order.backHome": "← Back to Home",
    "order.downloadInvoice": "Download Invoice",
    "order.dataUsage": "Data Usage",
    "order.used": "Used",
    "order.loading": "Loading order...",
    "order.notFound": "Order not found",

    // Profile extras
    "profile.emailVerified": "Email verified",
    "profile.emailNotVerified": "Email not verified",
    "profile.resendVerification": "Resend verification",
    "profile.verificationSent": "Verification email sent!",

    // Cookie consent
    "cookie.title": "Cookie usage",
    "cookie.description": "We use cookies for authentication, language preferences and analytics. We do not use cookies for advertising.",
    "cookie.accept": "Accept",
    "cookie.decline": "Decline",
    "cookie.learnMore": "Learn more",

    // eSIM Installation Guide
    "install.badge": "Installation Guide",
    "install.title": "How to install your eSIM",
    "install.subtitle": "Follow the simple steps below to activate your eSIM on iPhone or Android.",
    "install.iphone.step1": "Open your email and find the order with the eSIM QR code.",
    "install.iphone.step2": "Go to Settings → Cellular (or Mobile Data) → Add Cellular Plan.",
    "install.iphone.step3": "Scan the QR code with your phone camera. Make sure you are connected to Wi-Fi.",
    "install.iphone.step4": "Tap \"Continue\" when prompted — choose \"Secondary\" or \"Travel\" as the label.",
    "install.iphone.step5": "Enable \"Data Roaming\" → Settings → Cellular → Your eSIM → Roaming.",
    "install.iphone.step6": "Select the eSIM as the line for Cellular Data. Done! You have internet.",
    "install.android.step1": "Open your email and find the order with the eSIM QR code.",
    "install.android.step2": "Go to Settings → Network & Internet → SIMs → Add eSIM (or + Add).",
    "install.android.step3": "Choose \"Scan QR code\" and scan the code from the email. Wi-Fi is required.",
    "install.android.step4": "Confirm the eSIM profile download by tapping \"Continue\" or \"Download\".",
    "install.android.step5": "Enable the new eSIM → Settings → Network & Internet → SIMs → Enable Roaming.",
    "install.android.step6": "Select the eSIM as the mobile data line. Done! Internet is active.",
    "install.tipsTitle": "Important Tips",
    "install.tip1": "Make sure the phone is connected to Wi-Fi during installation.",
    "install.tip2": "Do not delete the eSIM once installed — you cannot reuse the QR code.",
    "install.tip3": "Always enable \"Data Roaming\" for the eSIM.",
    "install.tip4": "The eSIM can be installed before travel — activate it when you arrive.",
    "install.troubleTitle": "Installation Problems?",
    "install.trouble1": "Make sure your device supports eSIM (iPhone XS+, Samsung S20+, Pixel 3+).",
    "install.trouble2": "Check that the phone is not carrier-locked.",
    "install.trouble3": "The QR code can only be scanned once — it cannot be reused.",
    "install.trouble4": "If it doesn't work, contact our support through the contact page.",
  },
} as const;

export type Locale = keyof typeof translations;
export type TranslationKey = keyof (typeof translations)["sq"];

export default translations;
